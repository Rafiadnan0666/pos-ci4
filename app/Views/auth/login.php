...
<head>
    ...
    <title>Login - Outdoor Gear Store</title>
...
<body class="bg-[#06B6D4] min-h-screen flex items-center justify-center p-4" style="font-family:Inter,system-ui,sans-serif;">
    <div class="w-full max-w-md">
        <div class="text-center mb-6">
            <span class="text-6xl block mb-2">🏔️</span>
            <h1 style="font-family:'Space Grotesk',Inter,sans-serif;font-weight:800;text-transform:uppercase;font-size:2.25rem;color:#fff;letter-spacing:-.02em;">OUTDOOR GEAR</h1>
            <p style="font-weight:700;color:#fff;opacity:.8;margin-top:.25rem;font-size:.875rem;">Sign in to your account</p>
        </div>

        <div style="background:#FFDE4D;border:4px solid #000;box-shadow:4px 4px 0px 0px rgba(0,0,0,1);padding:1.5rem;">
            <?php if (session()->has('error')): ?>
            <div style="background:#EF4444;color:#fff;border:4px solid #000;padding:.75rem;margin-bottom:1rem;font-weight:700;font-size:.875rem;display:flex;align-items:center;gap:.5rem;"><span>✕</span> <?= session('error') ?></div>
            <?php endif; ?>
            <?php if (session()->has('message')): ?>
            <div style="background:#22C55E;border:4px solid #000;padding:.75rem;margin-bottom:1rem;font-weight:700;font-size:.875rem;display:flex;align-items:center;gap:.5rem;"><span>✓</span> <?= session('message') ?></div>
            <?php endif; ?>
            ...
            <form action="<?= base_url('login') ?>" method="POST">
                <div class="space-y-4">
                    ...
                    <div style="display:flex;align-items:center;gap:.5rem;">
                        <input type="checkbox" name="remember" id="remember" style="width:1.25rem;height:1.25rem;border:4px solid #000;" />
                        <label for="remember" style="font-weight:700;font-size:.875rem;">Remember me</label>
                    </div>
                    <button type="submit" style="font-family:'Space Grotesk',Inter,sans-serif;font-weight:700;text-transform:uppercase;font-size:1rem;padding:.75rem 1.5rem;border:4px solid #000;box-shadow:4px 4px 0px 0px rgba(0,0,0,1);cursor:pointer;background:#06B6D4;color:#fff;width:100%;"
                            ...
                            onmouseout="this.style.boxShadow='4px 4px 0px 0px rgba(0,0,0,1)';this.style.translate='0'">SIGN IN</button>
                </div>
            </form>
            ...
            <p style="text-align:center;font-size:.875rem;font-weight:700;">
                No account?
                <a href="<?= base_url('register') ?>" style="text-decoration:underline;padding:0 .25rem;">Register</a>
            </p>
        </div>
        ...
        <p style="text-align:center;margin-top:.5rem;color:#fff;font-size:.75rem;opacity:.7;">Demo: admin@outdoor.com / password123</p>
    </div>
</body>
</html>
