        ...
        <table class="w-full text-sm">
            ...
            <tbody>
                ...
                <tr class="border-b-2 border-black">
                    <td class="py-2 font-bold"><?= esc($item->name ?? 'Item #' . $item->product_id) ?><?= !empty($item->variant_label) ? ' <span class="text-xs opacity-60">' . esc($item->variant_label) . '</span>' : (!empty($item->size) ? ' <span class="text-xs opacity-60">(' . esc($item->size) . ')</span>' : '') ?></td>
